...
- Layout IDs were used as recipes, not mandatory full-slide clones.
...
- Slide sequence should not mirror the V1 sample template simply because the
  sample slides exist. Similarity to the V1 sample is acceptable only when the
  user's content has the same page intent and material shape.
...
- For IDT/Inspur decks, prefer template-derived composition over from-scratch generation. If the company template is available, reuse its master chrome and compose from registered components/recipes instead of rebuilding the whole package with PptxGenJS.
